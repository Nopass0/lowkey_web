...
): response is Extract<AuthResponse, { token: string; user: AuthUser }> {
  ...
  persist(
    (set) => ({
      ...
      verifyOtp: async (login, code) => {
        ...
        if (!hasAuthPayload(res)) throw new Error("Unexpected response");
        set({ isAuthenticated: true, user: res.user, token: res.token });
      },
